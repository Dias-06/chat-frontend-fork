...
export const ChatListItem: React.FC<ChatListItemProps> = ({
  ...
  const menuItems = (
    <>
      ...
      <ContextMenuItem iconName={isRead ? "MessageUnread" : "MessageRead"} onClick={() => console.log('Прочтение', id)}>
            ...
            <div className="flex-1 ml-3 min-w-0">
              <div className="flex justify-between items-baseline">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  {isMuted && (
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-gray">
              ...
              <div className="flex justify-between items-end mt-1">
                ...
                <div className="flex items-center shrink-0 mb-1 gap-1.5">
                  {/* Статус вашего последнего сообщения (исходящее) */}
                  <div className="flex items-center">
                    {isRead ? (
                      <IconRead className="text-primary" />
                    ) : isSent ? (
                      <IconSent className="text-gray" />
                    ) : null}
                  </div>

                  {/* Счетчик новых входящих сообщений (не зависит от статуса исходящего) */}
                  {unreadCount > 0 && (
