...
type ChatListItemProps = {
  ...
  wasOnlineAt?: number;
  isActive?: boolean; 
  isRead?: boolean;   // Для исходящих: прочитано ли получателем
  isPinned?: boolean; 
  isMuted?: boolean; 
  isSent?: boolean;   // Флаг: последнее сообщение в чате — моё
  showDivider?: boolean; 
...
export const ChatListItem: React.FC<ChatListItemProps> = ({
  ...
  const menuItems = (
    <>
      ...
      <ContextMenuItem iconName={isMuted ? "BellOn" : "BellOff"} onClick={() => console.log('Уведомления', id)}>
        {isMuted ? "Включить уведомления" : "Выключить уведомления"}
      </ContextMenuItem>
      <ContextMenuItem iconName="PinOff" onClick={() => console.log('Закреп', id)}>
        {isPinned ? "Открепить чат" : "Закрепить чат"}
      </ContextMenuItem>
      {/* Если есть непрочитанные, предлагаем "Прочитать", если нет — "Пометить непрочитанным" */}
      <ContextMenuItem iconName={unreadCount > 0 ? "MessageRead" : "MessageUnread"} onClick={() => console.log('Прочтение', id)}>
        {unreadCount > 0 ? "Пометить прочитанным" : "Пометить непрочитанным"}
      </ContextMenuItem>
            ...
            <div className="flex-1 ml-3 min-w-0">
              <div className="flex justify-between items-baseline">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  {isMuted && (
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-gray/60">
                ...
                <div className="flex items-center gap-1.5 ml-2 shrink-0">
                  {isPinned && (
                    <span className="opacity-40">
                      <svg width="14" height="14" viewBox="0 0 25 18" fill="none">
              ...
              <div className="flex justify-between items-end mt-1">
                ...
                <div className="flex items-center shrink-0 mb-1">
                  {/* ПРИОРИТЕТ 1: Входящие сообщения (счетчик) */}
                  {unreadCount > 0 ? (
                    ...
                    /* ПРИОРИТЕТ 2: Мои исходящие (статус доставки/прочтения) */
                    isSent && (
                      <div className="flex items-center">
                        {isRead ? (
                          <IconRead className="text-primary w-5 h-5" />
                        ) : (
                          <IconSent className="text-gray w-5 h-5" />
                        )}
                      </div>
                    )
                  )}
